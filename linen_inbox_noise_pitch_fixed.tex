...
\begin{frame}{The ask}
  \vspace{0.25cm}
  \begin{block}{Approve the first safe step}
    Let me roll out the Gmail label and filter system first, measure the before/after inbox noise, and use that win as the base for the next automation stage.
  \end{block}
  \vspace{0.45cm}
  \begin{center}
  \begin{tikzpicture}[node distance=0.55cm]
    \node[cardwhite,minimum width=3.1cm,minimum height=1.35cm] (a) {\bfseries No new platform\\\small uses Gmail first};
    \node[cardwhite,right=of a,minimum width=3.1cm,minimum height=1.35cm] (b) {\bfseries No customer risk\\\small no auto-send};
    \node[cardwhite,right=of b,minimum width=3.1cm,minimum height=1.35cm] (c) {\bfseries Clear next step\\\small n8n roadmap};
    \draw[arrow] (a) -- (b);
    \draw[arrow] (b) -- (c);
  \end{tikzpicture}
  \end{center}
